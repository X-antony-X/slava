...
import { useQuery } from '@tanstack/react-query'; // إضافة React Query
...
const CategoryPage = () => {
  ...
  const { data: rawProducts = [], isLoading: loading } = useQuery({
    queryKey: ['products', categoryName, filterType], 
  ...
  const products = useMemo(() => {
    ...
    return result;
  }, [rawProducts, selectedPrice, selectedSize, selectedColor, activeSort, filterType]);
            ...
            {['All Prices', 'Under 1,000', '1,000 - 3,000', 'Over 3,000'].map(p => {
              ...
              return (
                <label key={p} className="flex items-center gap-3 cursor-pointer group">
                  ...
                  />
                  <span className={`text-[15px] ${selectedPrice === value ? 'font-bold underline underline-offset-4' : 'text-gray-600'}`}>
                    {p}
                  </span>
                </label>
              );
    ...
    <div className="bg-white min-h-screen font-sans relative text-[#121212]">
      ...
      {cartItems?.length > 0 && (
        <div className="fixed bottom-20 left-4 right-4 z-50 md:hidden">
          <button 
            onClick={() => handleWhatsAppOrder(cartItems, totalPrice)}
